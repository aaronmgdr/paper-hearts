* [x] use service workers for syncing data
* [x] add notifications
* [x] app should be installable
* [x] add dev way to delete data
* [x] ui fixes on the on boarding (form sizes )
* [x] add touch id / face id instead of pin (if possible)
* [x] deploy
* [x] ADD README
* [x] add UI tests

* [x] use caching api so app really works offline
* [x] when relinking... after relinking is done take the user who showed the qr code a screen where they can choose to share the past entries with their partner. if they accept send the bundle over the current websocket connection. where it will be sent to the partner. 
* [x] privacy policy
* [ ] show real version in the settings (from pkg json).

* [ ] can we optimize our https://github.com/jedisct1/libsodium.js
* [ ] 
* [ ] dont store entries in plain text
* [x] see why notifications dont seem to show up
* [ ] name partner
